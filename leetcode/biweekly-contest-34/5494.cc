 ...
 *   输出：0
 *   解释：没有办法只用 3 单位的汽油从 0 到达 2 。因为最短路径需要 4
 *   单位的汽油。
 *   示例 4 ：
 *
...
public:
    int countRoutes(vector<int>& locations, int start, int finish, int fuel) {
      vector<int>& loc = locations;
      int n = loc.size();
      
      vector<vector<int64_t>> dp(n, vector<int64_t>(fuel + 1, -1));
      
      dp[start][fuel] = 1;
      
      int64_t iResult = 0;
      for (int i = fuel; i >= 0; i--)
        iResult = (iResult + dfs(loc, finish, i, fuel, dp)) % m_mod;
      
      return iResult;
    }
  
private:
  int64_t dfs(const vector<int>& loc, int pos, int cur, int fuel, vector<vector<int64_t>>& dp) {
    if (dp[pos][cur] != -1)
      return dp[pos][cur];
    ...
    for (int i = 0; i < loc.size(); i++) {
      if (i == pos)
        continue;
      int dist = std::abs(loc[i] - loc[pos]);
      if (cur + dist <= fuel) {
        iResult = (iResult + dfs(loc, i, cur + dist, fuel, dp)) % m_mod;
      }
    }
    dp[pos][cur] = iResult;
